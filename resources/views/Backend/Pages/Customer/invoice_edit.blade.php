               ...
               <form id="form-data" action="{{route('admin.customer.invoice.update_invoice')}}" method="post">@csrf
                  ...
                  <div class="form-group mb-2">
                     ...
                     <select type="text" id="customer_name" name="customer_id" class="form-control select2" style="width:100%">
                        <option>Select</option>
                        @foreach ($users as $customerItem)
                           @foreach ($data as $invoice)
                                 @if ($invoice->customer->id == $customerItem->id)
                                    <option value="{{ $customerItem->id }}" selected>{{ $customerItem->name }}</option>
                                 @else
                        ...
                        @foreach ($products as $product)
                           <div class=" col-sm-6 py-1">
                              <div class="product-list-box card p-2">
                                 ...
                                 <img src="{{ asset(request()->host().'/uploads/product/' . $product->product_image[0]->image ) }}" class="img-fluid" alt="work-thumbnail" style="height: 100px; width: 100px; ">
                                 </a>
                                 <div class="detail">
                                 <h6 class="font-size-10 mt-2"><a href="#" onclick="__get_product({{ $product->id }})" class="text-dark">{{ Illuminate\Support\Str::limit($product->title, 30) }}</a> </h6>
   ...
   $.each(products, function(index, product) {
      ...
      products_container.append(
            `<div class="col-sm-6 py-1">
               <div class="product-list-box card p-2">
                  <a href="javascript:void(0);" onclick="__get_product(${product.id})">
                        <img src="${newImageUrl}" class="img-fluid" alt="work-thumbnail" style="height: 100px; width: 100px;">
                  </a>
                  <div class="detail">
                        <h6 class="font-size-10 mt-2"><a href="#" onclick="__get_product(${product.id})" class="text-dark">${__get_short_string (product.title,30)}</a></h6>
                        <span>Price: ${ product.price}</span><br>
                        <span>Stock: ${product.qty}</span>
                  </div>
               </div>
