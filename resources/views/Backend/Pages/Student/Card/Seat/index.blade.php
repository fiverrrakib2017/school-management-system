                ...
                success: function(response) {
                    ...
                    if (Array.isArray(response.data) && response.data.length > 0) {
                        response.data.forEach(function(data) {
                            ...
                            html += '<td><input type="checkbox"  value="' + data.id +
                                '" name="student_ids[]" class=" student-checkbox"></td>';

                            html += '<td>' + (_number++) + '</td>';

                            var photo = data.photo ?
                                '<img src="' + "{{ asset('uploads/photos/') }}/" + data.photo +
                                '"style="width: 50px; height: 50px; border-radius: 50%;">' :
                                '<img src="' + "{{ asset('uploads/photos/avatar.png') }}" +
                                '"  style="width: 50px; height: 50px; border-radius: 50%;">';
        ...
        $(document).on('click', '#printBtn', function() {
            ...
            setTimeout(() => {
                ...
                if (selectedIds.length > 0) {
                    var url =
                        "{{ route('admin.student.seat_plan_print', ['exam_id' => ':exam_id', 'student_ids' => ':student_ids']) }}";
                ...
                print_button.html(`<i class="fas fa-print"></i> Generate`);
